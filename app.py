...
// Стартова зона (коло)
var startCircle = L.circle([48.3794,31.1656], {
    radius: 50000,
    color: "black",
    fillColor: "yellow",
    fillOpacity: 0.4
}).addTo(map).bindPopup("Стартова зона хімічного забруднення");

// Додаємо до drawnItems і layersMap для збереження
drawnItems.addLayer(startMarker);
drawnItems.addLayer(startCircle);
layersMap[L.stamp(startCircle)] = startCircle;
...
st.sidebar.caption("ОФІС CBRN v4.1")
