...
div[data-testid="stButton"] button, div[data-testid="stLinkButton"] a {background-color: #ffcc00 !important; color: #000 !important; border: none !important; width: 100% !important; font-weight: bold !important; font-size: 12px !important; border-radius: 4px !important; padding: 8px 12px !important; display: block !important; text-decoration: none !important; line-height: 1.2 !important;}
...
map.on(L.Draw.Event.CREATED, function(e){
    ...
    if(type==='marker'){
        ...
        layer.on('click', function(){var coords=layer.getLatLng().lat.toFixed(6)+", "+layer.getLatLng().lng.toFixed(6); layer.bindPopup("Координати:<br>"+coords).openPopup();});
    } else {
        if(layer instanceof L.Circle && layer.options.radius>=50000){layer.setStyle({color:'black', fillColor:'yellow', fillOpacity:0.4, weight:2});}
        else if(layer instanceof L.Circle){layer.setStyle({color:'black', fillColor:'orange', fillOpacity:0.8, weight:2});}
        else{setStyle(layer);}
    }
    drawnItems.addLayer(layer);
});

function addText(){var textMarker=L.marker(map.getCenter(), {icon:L.divIcon({className:'text-label', html:'Текст на карті', iconSize:[100,20]})}).addTo(drawnItems);}
function saveMap(){html2canvas(document.getElementById('map')).then(function(canvas){var link=document.createElement('a'); link.download='map_snapshot.png'; link.href=canvas.toDataURL(); link.click();});}
</script>

<button onclick="addText()">Додати текст на карту</button>
<button onclick="saveMap()">Завантажити скріншот карти</button>
"""
