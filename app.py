...
.main-title {color:#ffcc00; text-align:center; font-size:35px; font-weight:bold; margin-top:-30px; margin-bottom:15px; text-transform:uppercase;}
...
div[data-testid="stButton"] button, div[data-testid="stLinkButton"] a {
    background-color:#ffcc00; color:#000; border:none; width:100%; font-weight:bold; font-size:12px; border-radius:4px; padding:8px 12px; display:block; text-decoration:none; line-height:1.2;
}
...
with col_left:
    ...
    st.info("💡 Клікніть на маркер для перегляду координат.")
...
<div id="map" style="height:750px; width:calc(100% - 6px); border-radius:8px;"></div>
<script>
...
map.on(L.Draw.Event.CREATED, function(e){
    ...
    if(type==='marker'){
        ...
        layer.bindPopup("Координати:<br>"+layer.getLatLng().lat.toFixed(6)+", "+layer.getLatLng().lng.toFixed(6));
    } else if(type==='circle') {
        layer.setStyle({color:'black', fillColor:'orange', fillOpacity:0.6, weight:2});
    } else {
        setStyle(layer);
    }
...
function addText(){
    ...
    if(text){
        ...
        L.marker(center,{icon:L.divIcon({className:'text-label', html:text})}).addTo(drawnItems);
    }
}
</script>
<div style="margin-top:5px;">
<button onclick="addText()">Додати текст на карту</button>
<button onclick="var htmlContent=document.getElementById('map').outerHTML;var blob=new Blob([htmlContent],{type:'text/html'});var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='map.html';a.click();">Завантажити карту у HTML</button>
</div>
"""
